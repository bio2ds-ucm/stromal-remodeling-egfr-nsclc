...
# equivalent to pData(expr_data)
pheno_data <- readRDS("./Data/H12O_TMA004_cohort/Processed/H12O_TMA004_q3norm_pData.rds")
...
write.xlsx(fit_stroma_res, rowNames = TRUE, file = "./Results/Tables/H12O_TMA004_stromal_subtypes_dgea_stroma.xlsx")
...
# Supp File with DGEA results
write.xlsx(fit_tumor_res, rowNames = TRUE, file = "./Results/Tables/H12O_TMA004_stromal_subtypes_dgea_tumor.xlsx")
...
contrasts_mrkr_stroma <- makeContrasts(
  c1_vs_other = clusterC1 - (clusterC2 + clusterC3) / 2,
  c2_vs_other = clusterC2 - (clusterC1 + clusterC3) / 2,
  c3_vs_other = clusterC3 - (clusterC1 + clusterC2) / 2,
...
mrkr_res_stroma <- list(
  `C1_E cluster` = c1_mrkr_res_stroma |> arrange(desc(logFC)),
  `C2_E cluster` = c2_mrkr_res_stroma |> arrange(desc(logFC)),
  `C3_E cluster` = c3_mrkr_res_stroma |> arrange(desc(logFC))
)

# Supp File with DGEA results
write.xlsx(mrkr_res_stroma, rowNames = TRUE, file = "./Results/Tables/H12O_TMA004_stromal_subtypes_markers_stroma.xlsx")
...
# List
mrkr_res_tumor <- list(
  `C1_E cluster` = c1_mrkr_res_tumor |> arrange(desc(logFC)),
  `C2_E cluster` = c2_mrkr_res_tumor |> arrange(desc(logFC)),
  `C3_E cluster` = c3_mrkr_res_tumor |> arrange(desc(logFC))
)
...
cta_gs_list <- lapply(
  ...
  FUN = function(set) {
    gene_vector <- cta_gs |>
      filter(gs_name == set & gene_symbol %in% colnames(expr_data)) |>
      select(gene_symbol) |>
      pull()
  }
)
...
save.image(file = "./Results/Intermediate/H12O_TMA004_cohort/1_H12O_TMA004_stroma_unsupervised_analysis.RData")
